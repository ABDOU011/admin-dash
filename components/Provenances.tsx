...
interface ProvenancesProps {
  ...
  setProvenance: React.Dispatch<
    React.SetStateAction<{ name: string; namear: string; numstreet: number }[]>
  >;
  streets: {
    [key: number]: {
      id?: number;
  ...
  setStreets: React.Dispatch<
    React.SetStateAction<{
      [key: number]: { name: string; namear: string; numberOfStops: number }[];
    }>
  >;
  stops: {
  ...
  return (
    <div>
      {provenances.map((provenance, index) => (
        <div key={index} className="flex flex-col gap-3 pb-5">
          <div className="flex flex-row gap-3 w-full">
            <div className=" flex flex-col justify-center items-start gap-[12px] w-full ">
              <label className="text-black"> Provenance Name</label>
              <input
                type="text"
                name="name"
            ...
            <div className=" flex flex-col justify-center items-start gap-[12px] w-full ">
              <label className="text-black"> Provenance Arabic Name</label>
              <input
                type="text"
                name="namear"
                placeholder="Provenance Arabic Name"
                value={provenance.namear}
                onChange={(e) => handleChange(index, e)}
                className="text-black pl-4 w-full h-[50px] border-[1px] border-[solid] border-[#E4E4E4] rounded-[16px]"
              />
            </div>
                  ...
                  >
                    <div className="flex flex-col justify-center items-center gap-[24px] w-[350px] h-[200px]  rounded-[16px] p-[24px]">
                      ...
                      <div className="flex flex-row items-end justify-end w-full gap-2">
                        <button
                          onClick={handleClose}
                          className="flex flex-row justify-center items-center p-0 gap-[10px] w-[120px] h-[56px] text-black border-[1px] border-[solid] border-[#E4E4E4] rounded-[16px]"
                        >
                          Cancel
                        </button>
                        <button
