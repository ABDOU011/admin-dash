...
const RouteForm: React.FC<CityFormProps> = ({ setLine, line }) => {
  const [activeOption, setActiveOption] = useState<string>(line?.type? line.type :"bus");
  const [activeOption2, setActiveOption2] = useState<string>(line?.stype? line.stype : "Rotation");
      ...
      <div className="flex flex-col gap-6 w-full">
        ...
        <div className=" flex flex-col justify-center items-start gap-[12px] w-full ">
          ...
          <Select
            ...
            required
            placeholder={searchResults.find((result) => result.value === Number(line?.city))?.label}
            options={searchResults}
            onChange={(NewValue) => { if (line) {
